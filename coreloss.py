# import MPP_core_loss_parameters as clp
import pandas as pd
import numpy as np
...
def splitloop(a, b):
    ...
    for qq in range(0, pt):
        ...
        # If we're only passing a 1D array maybe minorLoop1 needs adjusted
...
def gsepwl(t, B, alpha, beta, k):
    ...
    # Double check that this is doing the same thing in Python that it does in MATLAB
    p = 0
...
def coreloss(tvs, B, alpha, beta, k):
    ...
    # Calculate core los if there is no error with the input data
    if not suppressFlag:
        ...
        if error == "None":
            ...
            # Skipped plotting the user's input
